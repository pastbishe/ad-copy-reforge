...
const StudioEmpty = () => {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      // Обновляем статус записи в photos на completed
      await updatePhotoImport(photoId, 'completed');
      
      // Обновляем прогресс
      setProgress(100);
      
      // Начинаем периодическую проверку завершенных импортов
      // n8n может заполнить photo_url позже, поэтому нужно проверять периодически
      ...
      let checkAttempts = 0;
      const maxCheckAttempts = 20; // Проверяем до 20 раз (около 1 минуты)
      const checkInterval = 3000; // Каждые 3 секунды
      
      checkIntervalRef.current = setInterval(async () => {
        checkAttempts++;
        
        try {
          // Проверяем завершенные импорты с photo_url
          const completedImports = await getUnprocessedCompletedPhotoImports(userId);
          
          if (completedImports.length > 0 && completedImports.some(imp => imp.photo_url)) {
            // Если найдены завершенные импорты с photo_url, редиректим на Studio
            if (checkIntervalRef.current) {
              clearInterval(checkIntervalRef.current);
            ...
            navigate('/studio', { 
              state: { 
                autoLoaded: true,
                hasCompletedImports: true
              } 
            });
          } else if (checkAttempts >= maxCheckAttempts) {
            // Если прошло много времени и импортов нет, просто редиректим на Studio
            if (checkIntervalRef.current) {
              clearInterval(checkIntervalRef.current);
              checkIntervalRef.current = null;
            }
          ...
          console.error('Ошибка проверки завершенных импортов:', error);
